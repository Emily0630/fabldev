...
gibbs_efficient_mm <- function(hash, m_prior = 1, u_prior = 1,
  ...
  for(s in 1:S){
    ...
    # counts_list <- hash$pattern_counts_by_record
    unique_weights <- exp(rowSums(ratio * unique_patterns, na.rm = TRUE))
    # counts_long <- hash$pattern_counts_by_record %>%
    #   unlist
    # weights_long <- exp(rowSums(ratio * unique_patterns, na.rm = TRUE)) %>%
    #   rep(., n2)

    weight_df$count <- hash$pattern_counts_by_record %>%
      unlist()
    ...
    while(TRUE){
      ...
      # pattern_weights <- lapply(counts_list, function(x){
      #   x * unique_weights
      # }) %>%
      #   unlist()

      #pattern_weights <- weights_long * counts_long

      #weight_df$weights[!is.na(weight_df$pattern)] <- pattern_weights
      #weight_df$weights[is.na(weight_df$pattern)] <- offset

      sampled_rows <- sapply(matchable, function(x){
        sample(c(0, weight_split[[x]]$rn),
               1,
               prob = c(offset, weight_split[[x]]$count * unique_weights))
      }) %>%
        unname()
